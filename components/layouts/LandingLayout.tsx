"use client";

import Image from "next/image";
import React from "react";
import { useRouter } from 'next/navigation'


interface LandingLayoutProps {
  children: React.ReactNode;
}

export default function LandingLayout({ children }: LandingLayoutProps) {

    const router = useRouter();

    const getStarted = () => {
        router.push("/signup");
      };

  return (
    <div className="relative bg-backImage min-h-[100vh]">
      <div className="pt-6 absolute inset-0 bg-primary bg-opacity-70 sm:px-6 xs:px-6 1xs:px-6">
        <div className="pl-8 flex">
            <Image height={30} width={100} src={"/logo_white.png"} alt="logo" />
        </div>
        <div className="flex items-center justify-around h-[95%]">
            <div className="md:w-[600px]">
                <div className="">
                    <p className="font-boldItalic text-6xl inline-block mr-3 text-splash">Smart</p><p className="inline-block 
                    font-extraBold text-6xl text-white"> Payroll for</p>
                    <p className="font-extraBold text-6xl text-white">Businesses</p>
                </div>
                <div className="md:w-80">
                    <p className="text-white text-lg font-regular">Start managing your workforce Salaries with
                        automation, precision and security. </p>
                </div>
                <button
                    onClick={getStarted}
                    className="bg-primary rounded-2xl text-white px-7 py-3 mt-10 
                    font-semi tracking-widest">
                    Get Started
                </button>
            </div>
            {children}
        </div>
      </div>
    </div>
  );
}
